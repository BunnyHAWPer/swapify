...
try {
    ...
    Write-Section 112 "FEATURE 4 - LIST CATEGORIES  (GET /products/categories)  <- DB + Open Food Facts counts"
    ...
    if ($catsExt -gt 0) {
        ...
        Write-Host "  (external_products=0 - OFF unavailable or disabled on the host; not failing the run)" -ForegroundColor Yellow
    }

    Write-Section "112a" "FEATURE 4 - CURATED-ONLY COUNTS  (GET /products/categories?external=false)"
    ...
    Write-Host "  external=false -> total_products=$(if ($c2) { [int]$c2.total_products } else { 0 }) (external_products=$dbOnlyExt)" -ForegroundColor Blue
    ...
    Write-Host "  offset=100 -> count=$(if ($o) { [int]$o.count } else { 0 })  (openfoodfacts rows present: $deepSrc)" -ForegroundColor Blue
    ...
    Write-Host "  external=false -> total=$(if ($o) { [int]$o.total } else { 0 }) external_count=$curExt" -ForegroundColor Blue
